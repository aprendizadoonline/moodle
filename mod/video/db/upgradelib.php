...
/**
 * Migrate video module data from 1.9 resource_old table to new video table
 * @return void
 */
function video_20_migrate() {
    global $CFG, $DB;
    ...
    foreach ($candidates as $candidate) {
        ...
        $video = new stdClass();
        $video->course       = $candidate->course;
        $video->name         = $candidate->name;
        $video->intro        = $intro;
        $video->introformat  = $introformat;
        $video->externalvideo  = $path;
        $video->timemodified = time();

       

        if (!$video = resource_migrate_to_module('video', $candidate, $video)) {
            continue;
        }
    }
