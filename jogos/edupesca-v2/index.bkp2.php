		...
		<script>
			var world, 
				...
				generateWorld = function() {
					...
					Crafty.n1 = Crafty.e("2D, DOM").attr({
						w: 453,
						h: 183,
						x: 100,
						y: -30,
					...
					Crafty.n2 = Crafty.e("2D, DOM").attr({
						w: 619,
						h: 228,
						x: 600,
						y: -100,
					...
					Crafty.n3 = Crafty.e("2D, DOM").attr({
						w: 708,
						h: 178,
						x: 1900,
						y: -50,
					...
					server.barco.crafty = Crafty.e("2D, DOM, Multiway, Keyboard, Box2D, barco_pcs").origin("center").attr({
						...
						server.barco.chave && stopTempo == false && (a.keyCode == Crafty.keys.A ? (player.moveBoat = -25, e(!0), left = !0) : a.keyCode == Crafty.keys.D && (player.moveBoat = 25, e(!0), right = !0), a.keyCode == Crafty.keys.W && server.barco.chave ? (player.moveVara = -1, r(!0)) : a.keyCode == Crafty.keys.S && server.barco.chave && (player.moveVara = 1, r(!0)))
					}).bind("KeyUp", function(a) {
						a.keyCode == Crafty.keys.A ? (player.moveBoat = 0, e(!1), left = !1) : a.keyCode == Crafty.keys.D && (player.moveBoat = 0, e(!1), right = !1), a.keyCode == Crafty.keys.W && server.barco.chave ? (player.moveVara = 0, r(!1)) : a.keyCode == Crafty.keys.S && server.barco.chave && (player.moveVara = 0, r(!1))
					}), 
				...
				server = {
					...
					win_pesca: function(e) {
						if (server.lct !== e.id) {
							...
							console.log(a);
							server.letra[server.letraSoma] = e.caractere;
							if (server.letra.length == server.LetraObj.pq[0].length || a.length > 1) {
								var palavra = "";
					...
					player: {
						...
						criarKarl: function(e) {
							server.barco.chave = !1, server.player.subir = !1, Crafty.sprite(server.karl2.largura, server.karl2.altura, server.karl2.imagem, server.karl2.mapa), server.karl = Crafty.e("2D, DOM, SpriteAnimation, Box2D, karl, Movable, Twoway, Mouse, Keyboard").attr({
								x: e,
								y: 130,
								z: 15
							}).css({
							...
							}).twoway(3).animate("PlayerMovingRight", 0, 0, 7).animate("PlayerMovingLeft", 0, 1, 7).animate("PlayerMovingStopRight", 1, 0, 1).animate("PlayerMovingStopLeft", 6, 1, 1), server.karl.bind("KeyDown", function(e) {
								return left && right ? (console.log("stop"), void server.karl.animate("PlayerMovingStopRight", 0)) : (e.keyCode === Crafty.keys.A && (server.karl.animate("PlayerMovingLeft", 25, -1), left = !0), void(e.keyCode === Crafty.keys.D && (server.karl.animate("PlayerMovingRight", 25, -1), right = !0)))
						...
						move: function() {
							if (!server.barco.chave && stopTempo == false) {
								...
								if (0 != player.mousePos) {
									...
									if (e.toFixed() > 0 && right) return server.karl.animate("PlayerMovingStopRight", 0), void(player.mousePos = 0);
