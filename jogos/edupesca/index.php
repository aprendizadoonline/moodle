<?php
	...
	$result = mysql_query("SELECT score, nivelGamepesca FROM user;");
	...
	$result  = mysql_query("SELECT id, mensagem, palavras, palpesca, tempo, imagem FROM gamepesca WHERE id={$nivel} ORDER BY ordem;") or die(mysql_error());
	if ($row = mysql_fetch_array($result)) {
		$palavra = $row['palavras'];
		$pImagem = htmlspecialchars($row['imagem']);
		$palPesca = htmlspecialchars($row['palpesca']);
		$pInformacao = htmlspecialchars($row['mensagem']);
	} else {
		die("ERROR 2");
	}
...
<html>
	<head>
		...
		<script type="text/javascript" src="js/Box2dWeb-2.1.a.4.js"></script>
		...
		<script>
			...
			var ColMaskPalavra  = ColCatAgua|ColCatAnzol|ColCatParede;
			...
			var EduPesca = function(config) {
				 ...
				 * @param EduPesca _this Referência ao *this* da instância. Essa função é executada em um escopo diferente, por isso a necessidade.
				 */
				this.showHelp = function() {
					// Preenche o conteúdo
					$("#modalMessageTitle").html(this.config.levelTitle);
					$("#modalMessageAction").html("Jogar!");
					$("#modalMessageBody")
						.addClass("text-center")
						.append($("<h2>").html(this.config.levelInfo));
					
					// Hack para as funções do listener acessarem o objeto this, já que estão em escopo diferente
					_this = this;
					
					// Escuta cliques no botão de jogar
					$("#modalMessageAction").click(function() {
						_this.running = true;
						_this.startTime = Date.now();
					});
				...
				this.createEntities = function() {
					...
					this.instanceEntities.encostaEsquerda = Crafty.e("2D, DOM, Image")
						...
						.image("images/encosta1.png", "no-repeat");
					...
					this.instanceEntities.encostaDireita = Crafty.e("2D, DOM, Image")
						...
						.image("images/encosta2.png", "no-repeat");
					...
					this.instanceEntities.campo = Crafty.e("2D, DOM, Image, Static")
						...
						.image("images/campo.png", "repeat-x")
						.lockX();
					...
					this.instanceEntities.sol = Crafty.e("2D, DOM, Image, Static")
						...
						.image("images/sol.png", "no-repeat")
						.lockX();
					...
					this.instanceEntities.aguaBaixo = Crafty.e("2D, DOM, Image, Static")
						...
						.image("images/agua-fundo.png", "repeat-x")
					...
					this.instanceEntities.pier = Crafty.e("2D, DOM, Image")
						...
						.image("images/pier.png", "no-repeat");
					...
					this.instanceEntities.barco = Crafty.e("2D, DOM, Image, Box2D, Barco")
						...
						.image("images/barco.png", "no-repeat")
						.box2d({
							bodyType: 'dynamic',
							density: 0.9,
							friction: 0,
					...
					this.instanceEntities.edu = Crafty.e("2D, DOM, Image, Box2D")
						...
						.image("images/edu.png", "no-repeat");
				...
				this.createJoints = function() {
					...
					this.bracoJoint = Crafty.box2D.revoluteJoint({
						...
						anchorB: [32/PTM_RATIO, 47/PTM_RATIO],
				...
				this.processText = function(text) {
					...
					if (finishState == 1) {
						...
						Crafty.e("2D, DOM, Text")
							.attr({ w: 300, h: 48, x: (this.config.gameWidth - 300)/2 - Crafty.viewport.x, y: (this.config.gameHeight - 48)/2 - Crafty.viewport.y })
				...
				this.postFinishMessage = function(state) {
					...
					if (state == 1) {
						...
						// TODO: Enviar resultado para servidor
					// Caso contrário
					} else {
						// Mostra o botão de continuar
						$("#ActionBtn")
							.css('display', 'block')
							.html('Continuar jogando')
							.click(function() {
								$.ajax({
									type: "POST",
									url: "user.php",
									data: "nivel=" + _this.config.backendNivel + "&score=" + (_this.endTime - _this.startTime),
			...
			$(function() {
				...
				game = new EduPesca({
					//= Informações do container do jogo
					gameContainer: '#game',
					gameWidth: $(window).width(),
					gameHeight: 600,//$(window).height(),
					
					//= Informações do nível
					levelTitle: "", // "Título do nível",
					levelInfo:  dbData.info,
					levelWord:  dbData.palavra,
					levelData:  dbData.opts,
					levelImg:   dbData.img,
	...
	<body>
		...
		<a href="painel.php" class="btn btn-primary">Acesso do Professor</a>
		
		<div class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="" aria-hidden="true" id="modalMessage">
